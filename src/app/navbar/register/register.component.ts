...
export class RegisterComponent implements OnInit {
  ...
  registerUser(modal) {
    if (!this.formValidation()) {
      ...
      this._authService.registerProfesional(this.register1).subscribe(
        resp => {
          console.log(resp)
          this.clearForm();
          this._toastr.success('La cuenta fue creada exitosamente. Ingrese a su email para activarla', 'Creacion Exitosa');
          modalSpiner.close();
        }, (error) => {
          console.log(error)
          modalSpiner.close();
          this._toastr.error('No se pudo crear la cuenta, porfavor reintente mas tarde', 'Fallo la Creacion');
        }
      )
    }
  }
